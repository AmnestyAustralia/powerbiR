get_request <- function(url, header, simplifyVector = FALSE) {
  ...
  if (inherits(resp, "try-error")) {
    stop(
      "The Power BI API returned an empty value or the value could not be parsed."
    )
  }
...
pbi_schema_types_infer <- function(vector) {
  if (!is.atomic(vector)) {
    stop("The argument must be an atomic vector")
  }

  if (inherits(vector, "Date")) {
    "DateTime"
  } else if (inherits(vector, "POSIXt")) {
    "DateTime"
  } else if (inherits(vector, "POSIXct")) {
    "DateTime"
  } else if (inherits(vector, "factor")) {
    "String"
  } else if (inherits(vector, "logical")) {
    "Boolean"
  } else if (inherits(vector, "numeric")) {
    "Double"
  } else if (inherits(vector, "integer")) {
    "Int64"
  } else {
    "String"
  }
...
pbi_row_push_few <- function(dt, group_id, dataset_id, table_name) {
  rowcount <- nrow(dt)
